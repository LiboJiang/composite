...
loss2<-function(par,t,y_P,snp.type,SNP){
  ...
  par.covar <- par[(snp.index*3+1):(snp.index*3+2)]
  AR1_P <- SAD1(par.covar,t,traits=1)
  ...
  for(ii in 1:snp.index){
    ...
    for(j in 1:dim(yy)[1]){
      ...
      y3_1<-fn_P(t2_1,par[((ii-1)*3+1):((ii-1)*3+3)])
      
      fy<-dmvnorm(y2,y3_1,AR2_P)
...
par0<-c( 578.4350435, 300.0188600 ,  0.1337533 ,  0.8945486 , 40)
H0<-optim(par0,loss1,t=dat_P_st$TIME,y_P=pheno,
...
for(i in n1:n2){#
  ...
  if(length(miss.snp)>0){
    ny_P_1 <- pheno[-miss.snp,]
  }else{
    ny_P_1 <-pheno
  } 
  
  NH0 <- optim(H0$par,loss1,t=dat_P_st$TIME,y_P=ny_P_1,
               method="BFGS",
  ...
  if(length(snp.type)==2)
    npar<-c(H0$par[1:3],H0$par[1:3],H0$par[4:5])
  
  if(length(snp.type)==3)
    npar<-c(H0$par[1:3],H0$par[1:3],H0$par[1:3],H0$par[4:5])
...
filename <- paste("par_len_del-",test3,".RData",sep="")
save(ret_len_del,file=filename)
